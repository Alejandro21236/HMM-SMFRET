...
# === Input files ===
topology_file = "/scratch/FRET_true/RNAbound/SYS_nowat.pdb"
trajectory_file = "/scratch/FRET_true/RNAbound/traj_3000ns.dcd"
...
time_per_frame_ns = 3000 / len(u.trajectory)
...
for i, ts in tqdm(enumerate(u.trajectory), total=len(u.trajectory)):
    # Time
    features["Time_ns"].append(i * time_per_frame_ns)
...
df.to_csv("/scratch/FRET_true/RNAbound/RNA.csv", index=False)
...
plt.grid(True)
plt.tight_layout()
plt.savefig("/scratch/FRET_true/RNAbound/fret_efficiency_vs_time.png")
plt.show()
...
plt.tight_layout()
plt.savefig("/scratch/FRET_true/RNAbound/fret_efficiency_histogram.png")
plt.show()
